...
export default function Process() {
  return (
    <section id="process" className="py-24 px-5 md:px-12 bg-dark2 border-y border-slate-200">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {processSteps.map((step, index) => {
            ...
            return (
              <div key={step.number} className="relative flex flex-col items-start bg-dark border border-slate-800/5 p-6 md:p-8 rounded-none hover:border-brand transition-all duration-300">
                {/* Connecting Line (Desktop) */}
                {index < 3 && (
                  <div className="hidden lg:block absolute lg:left-[110px] lg:right-[-20px] top-12 border-t border-dashed border-slate-200 z-0" />
                )}
                ...
                <h3 className="font-display font-black text-base uppercase tracking-tight text-slate-800 mb-3">
                  {step.title}
                </h3>

                <p className="text-xs text-slate-800/50 leading-relaxed mb-5">
                  {step.description}
                </p>

                <ul className="flex flex-col gap-1.5 border-t border-slate-200 pt-4 w-full">
                  {step.substeps.map((sub, i) => (
                    <li key={i} className="text-[11px] text-slate-800/45 flex items-start gap-1.5">
