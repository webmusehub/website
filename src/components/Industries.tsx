...
export default function Industries() {
  return (
    <section id="industries" className="py-24 px-5 md:px-12 bg-dark2 border-y border-slate-200">
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-xs md:text-sm text-slate-800/50 uppercase tracking-widest mt-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
          {industries.map((ind) => (
            <div
              key={ind.name}
              className="bg-dark border border-slate-200 rounded-none p-5 text-center flex flex-col items-center justify-center transition-all duration-300 hover:border-brand select-none"
            >
              <span className="text-2xl mb-2.5 filter drop-shadow">{ind.emoji}</span>
              <span className="text-xs font-black uppercase tracking-wider text-slate-800">{ind.name}</span>
            </div>
          ))}
        </div>
      </div>
