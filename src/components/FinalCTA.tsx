...
export default function FinalCTA() {
  ...
  return (
    <section id="final-cta" className="relative py-24 px-5 md:px-12 bg-dark overflow-hidden border-t border-white/10">
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none text-white mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start my-12">
          ...
          <div className="lg:col-span-8 bg-dark2 border border-white/10 rounded-none p-6 sm:p-8 shadow-xl">
            {/* Steps Progress Header */}
            <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-5">
              <div className="flex items-center gap-2">
                <span className={`w-5 h-5 rounded-none flex items-center justify-center text-[10px] font-black leading-none ${step >= 1 ? 'bg-brand text-white' : 'bg-white/10 text-white/40'}`}>1</span>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${step === 1 ? 'text-white' : 'text-white/45'}`}>Details</span>
              </div>
              <ChevronRight size={12} className="text-white/20" />
              <div className="flex items-center gap-2">
                <span className={`w-5 h-5 rounded-none flex items-center justify-center text-[10px] font-black leading-none ${step >= 2 ? 'bg-brand text-white' : 'bg-white/10 text-white/40'}`}>2</span>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${step === 2 ? 'text-white' : 'text-white/45'}`}>Scheduler</span>
              </div>
              <ChevronRight size={12} className="text-white/20" />
              <div className="flex items-center gap-2">
                <span className={`w-5 h-5 rounded-none flex items-center justify-center text-[10px] font-black leading-none ${step >= 3 ? 'bg-brand text-white' : 'bg-white/10 text-white/40'}`}>3</span>
                <span className={`text-[10px] font-bold uppercase tracking-wider ${step === 3 ? 'text-white' : 'text-white/45'}`}>Success</span>
              </div>
            </div>
              ...
              <form onSubmit={handleNextStep} className="flex flex-col gap-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-dark border border-white/10 hover:border-white/20 focus:border-brand focus:ring-0 rounded-none px-4 py-3 text-sm text-white placeholder-white/20 outline-none transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-white/70 uppercase tracking-widest mb-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-dark border border-white/10 focus:border-brand focus:ring-0 rounded-none px-4 py-3 text-sm text-white outline-none transition-all"
                  >
                    {services.map((s) => (
                      <option key={s.id} value={s.title} className="bg-dark text-white">
                        {s.title}
                      </option>
                    ))}
                  </select>
                ...
                <button
                  type="submit"
                  className="bg-brand hover:bg-white hover:text-black text-white font-bold rounded-none px-4 py-4 text-center flex items-center justify-center gap-2 mt-2 font-display uppercase text-xs tracking-wider transition-all cursor-pointer"
                >
            ...
            {step === 2 && (
              <div>
                {/* Date Selection */}
                <div className="mb-6">
                  <label className="block text-[10px] font-bold text-white/70 uppercase tracking-widest mb-3.5 flex items-center gap-1.5">
                ...
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="border border-white/15 hover:bg-white/5 text-white font-bold rounded-none px-4 py-4 text-center flex-1 font-display uppercase text-xs tracking-wider transition-all cursor-pointer"
                  >
                    Back
                  </button>
                  <button
                    ...
                    className="bg-brand hover:bg-white hover:text-black text-white font-bold rounded-none px-6 py-4 text-center flex-1 flex items-center justify-center gap-2 font-display uppercase text-xs tracking-wider transition-all cursor-pointer"
                  >
                    Confirm & Book Call
                    <CheckCircle size={14} />
                  </button>
            ...
            {step === 3 && (
              <div className="flex flex-col items-center text-center py-8">
                ...
                <h3 className="font-display font-black text-2xl uppercase tracking-tight text-white mb-2">Strategy Session Booked!</h3>
                ...
                <div className="bg-black border border-white/10 rounded-none p-4 text-left w-full max-w-sm mb-8">
                  <div className="text-[9px] text-brand uppercase font-bold tracking-widest">Scheduled Spot</div>
                  <div className="text-sm font-black text-white mt-1 uppercase tracking-wider">
                    {businessDays.find((d) => d.id === selectedDate)?.formatted || selectedDate}
                  </div>
                  <div className="text-xs text-white/50 mt-1 uppercase tracking-wider">{selectedSlot} (Indian Standard Time)</div>
                </div>
                <button
                  type="button"
                  onClick={resetFlow}
                  className="bg-white/5 hover:bg-white/10 text-white font-bold rounded-none px-5 py-3.5 text-[10px] uppercase tracking-widest transition-colors cursor-pointer"
                >
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-dark2 border border-white/10 rounded-none p-6">
              <h4 className="font-display font-black text-sm uppercase tracking-wider text-white mb-4">Direct Communication</h4>
              <div className="flex flex-col gap-4 text-xs text-white/50">
                <div className="flex flex-col">
                  ...
                  <span className="text-white font-bold">Vidyanagar, Hubballi, Karnataka - 580021</span>
                </div>
                <div className="flex flex-col">
            ...
            <div className="bg-dark/60 border border-white/10 rounded-none p-6">
              <h4 className="font-display font-black text-xs text-white uppercase tracking-wider mb-4 flex items-center justify-between">
                <span>Active Bookings</span>
                <span className="bg-brand/10 text-brand text-[10px] px-2 py-0.5 rounded-none font-black">{bookings.length}</span>
              </h4>
              <div className="flex flex-col gap-3">
                {bookings.map((booking) => (
                  <div key={booking.id} className="bg-white/3 border border-white/10 rounded-none p-3.5 text-left relative group">
                    <button
                      onClick={() => handleDeleteBooking(booking.id)}
                      className="absolute top-2.5 right-2 text-white/30 hover:text-rose-400 p-1 rounded-none hover:bg-rose-500/10 transition-all opacity-0 group-hover:opacity-100"
                      title="Cancel Booking"
                    >
                      <Trash2 size={13} />
                    </button>
                    <div className="text-[9px] font-bold text-brand uppercase tracking-widest">{booking.service}</div>
                    <div className="text-xs font-black text-white mt-1 leading-tight uppercase tracking-tight">{booking.name}</div>
                    <div className="text-[10px] text-white/50 mt-1 uppercase font-bold">{booking.date} · {booking.timeSlot}</div>
                  </div>
                ))}
                {bookings.length === 0 && (
                  <div className="text-[11px] text-white/40 uppercase tracking-wide font-medium py-4 text-center">
